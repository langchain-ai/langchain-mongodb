...
"""
    ...
    docs.append(("api_docs.rst", index_doc + index_autosummary))
    return docs
...
def _build_index(dirs: List[str]) -> None:
    integrations = sorted(dirs)
    doc = """# LangChain MongoDB Python API Reference

Welcome to the LangChain MongoDB Python API reference. This is a reference for all
Langchain MongoDB packages.

For user guides see [https://python.langchain.com](https://python.langchain.com).
"""
    if integrations:
        integration_headers = [
            " ".join(x.title() for x in dir_.split("-")) for dir_ in integrations
        ]
        integration_tree = "\n".join(
            f"{header_name}<{dir_.replace('-', '_')}/api_docs>"
            for header_name, dir_ in zip(integration_headers, integrations)
        )

        integration_grid = ""
        integrations_to_show = integrations
...
def main(dirs: Optional[list] = None) -> None:
    ...
    if not dirs:
        dirs = [dir_ for dir_ in os.listdir(ROOT_DIR / "libs")]
    for dir_ in dirs:
