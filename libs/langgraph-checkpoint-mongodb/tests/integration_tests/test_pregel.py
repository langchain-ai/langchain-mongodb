...
@pytest.mark.parametrize("checkpointer_name", ALL_CHECKPOINTERS_SYNC)
@pytest.skip.xfail('Interrupt does not work with transactions')
...
@pytest.mark.parametrize("checkpointer_name", ALL_CHECKPOINTERS_SYNC)
@pytest.mark.xfail('Interrupt does not work with transactions')
def test_root_graph(
    deterministic_uuids: MockerFixture,
    request: pytest.FixtureRequest,
    checkpointer_name: str,
        ...
        },
        {
            "tools": [
                _AnyIdToolMessage(
                    ...
                    name="search_api",
                    tool_call_id="tool_call123",
                    # id="00000000-0000-4000-8000-000000000033",  # TODO - We are off by one because pymongo.synchronous.client_session._ServerSession.session_id uses uuid4
                )
            ]
        },
        {
