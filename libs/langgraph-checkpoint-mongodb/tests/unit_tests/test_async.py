...
async def saver():  #  -> Generator[AsyncMongoDBSaver, None, None]:
    # Prep by clearing collections if they exist
...
async def test_asearch(saver, input_data: dict[str, Any]) -> None:
    # save checkpoints
    await saver.aput(
        input_data["config_1"],
        input_data["chkpnt_1"],
...
async def test_null_chars(saver, input_data: dict[str, Any]) -> None:
