...
def setup_test() -> tuple[AsyncCollection, MongoDBAtlasVectorSearch]:
    ...
    async_client = AsyncMongoClient(CONNECTION_STRING)
    async_coll = async_client[DB_NAME][COLLECTION_NAME]

    async_vs = PatchedMongoDBAtlasVectorSearch(
        async_coll, # type: ignore[arg-type]
    ...
    return async_coll, async_vs




class TestMongoDBAtlasHybridSearchRetrieverAsync(RetrieversIntegrationTests):
    @property
    ...
    @pytest.mark.xfail(reason="Testing with Async MongoClient requires async tests.")
    async def test_k_constructor_param(self) -> None: # type: ignore[override]
        """
    ...
    @pytest.mark.xfail(reason="Testing with Async MongoClient requires async tests.")
    async def test_invoke_with_k_kwarg(self, retriever: BaseRetriever) -> None: # type: ignore[override]
        """
    ...
    @pytest.mark.xfail(reason="Invoke does not work for AsyncMongoClient.")
    async def test_invoke_returns_documents(self, retriever: BaseRetriever) -> None: # type: ignore[override]
        pass
